    ...
    public class GlovesData : Item_Equipment
    {
        public override Item Create()
        {
            ...
            if (ItemType.Gloves == Type)
            {
                ...
                float.TryParse(CsvManager.Instance.gloves.GetData($"{rate}", "최소"), out float min);
                float.TryParse(CsvManager.Instance.gloves.GetData($"{rate}", "이속"), out float speed);
                int.TryParse(CsvManager.Instance.gloves.GetData($"{rate}", "체력"), out int hp);
                int.TryParse(CsvManager.Instance.gloves.GetData($"{rate}", "방어력"), out int defense);
                int.TryParse(CsvManager.Instance.gloves.GetData($"{rate}", "치확"), out int critical);
                float.TryParse(CsvManager.Instance.gloves.GetData($"{rate}", "공속"), out float attackSpeed);
                int.TryParse(CsvManager.Instance.gloves.GetData($"{rate}", "스테미나"), out int stemina);
                float.TryParse(CsvManager.Instance.gloves.GetData($"{rate}", "장비획득률 증가"), out float equipRate);
                float.TryParse(CsvManager.Instance.gloves.GetData($"{rate}", "공격력"), out float damage);
                int.TryParse(CsvManager.Instance.gloves.GetData($"{rate}", "마나"), out int mana);

                // 장비 등급 숫자로 주스텟과 설명 추가
                // 주스텟
                createitem.mEffect.AttackSpeed = Mathf.Round(Random.Range(min, attackSpeed) * 100f) / 100f;
                // 설명
                createitem.Name = CsvManager.Instance.gloves.GetData($"{rate}", "이름");
