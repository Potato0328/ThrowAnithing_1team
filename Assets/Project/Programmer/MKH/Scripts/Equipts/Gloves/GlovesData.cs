...
namespace MKH
{
    ...
    public class GlovesData : Item
    {
        public override Item Create()
        {
            ...
            // Rare : 2
            int rate = (int)Rate;
            var csv = CsvManager.Instance.gloves;
            ...
            // 주스텟
            item.mEffect.AttackSpeed = Mathf.Round(Random.Range(min, attackSpeed) * 100f) / 100f;
            // 설명
            item.Name = csv.GetData($"{rate}", "이름");
            item.Description = $"공격속도 + {item.mEffect.AttackSpeed * 100f}%";
