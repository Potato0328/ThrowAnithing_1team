...
namespace MKH
{
    ...
    public class GlassesData : Item
    {
        public override Item Create()
        {
            ...
            // Rare : 2
            int rate = (int)Rate;
            var csv = CsvManager.Instance.glasses;
            ...
            // 주스텟
            item.mEffect.Critical = Random.Range(min, critical);
            // 설명
            item.Name = csv.GetData($"{rate}", "이름");
            item.Description = $"치명타 확률 + {item.mEffect.Critical}%";
